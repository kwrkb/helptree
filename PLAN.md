...
## Phase 8: macOS ディスカバリーテスト + パーサー改善

macOS 環境でディスカバリーテストを実施し、環境差異の把握とパーサー改善を行った。

- [x] macOS ディスカバリーテスト実行（1,174 コマンド）
- [x] テスト進捗ログ追加（コマンドごとのステータス出力）
- [x] パーサーハングのタイムアウトガード追加（10秒でスキップ）
- [x] 入力サイズガード追加（1MB超を切り詰め、PARSE_TIMEOUT 解消）
- [x] 埋め込み usage 検出（`usage is` / 行中 `usage:` 対応 — top, zic 等）
- [x] パイプ区切り引数付きオプション対応（`[-n lines | -c bytes]` — head 等）
- [x] `<angle>` 記法の引数サポート（`[-c <mode>]` — top 等 macOS ツール）
- [x] スペース付きブラケット正規化（`[ --opt ]` → `[--opt]` — zic 等）

### 結果

| ステータス | Before (macOS) | After (macOS) | 差分 |
|-----------|----------------|---------------|------|
| OK | 835 (75.4%) | 870 (78.2%) | +35 |
| EMPTY | 272 | 242 | -30 |
| SKIP | 67 | 61 | -6 |
| PARSE_TIMEOUT | 3 | 0 | -3 |

> WSL (Phase 7) とは環境差が大きい: macOS は BSD ツール中心で `usage:` 行のみの簡素なヘルプが多い。
> WSL は GNU ツール中心で構造化されたヘルプが多く、OK 率が高い (88.0%)。

### discover_test.go の改善

- コマンドごとの進捗ログ（`[N/total] cmd ... -> OK/EMPTY/SKIP/PARSE_TIMEOUT`）
- パーサーのタイムアウト検出（goroutine + select で 10 秒制限）
- リアルタイムで進捗を追跡可能に
